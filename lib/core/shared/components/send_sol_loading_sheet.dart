...
class SendSolLoadingSheet extends StatefulWidget {
  final String receiverAddress;
  final double amountInSol;
  const SendSolLoadingSheet({
    super.key,
  ...
  @override
  State<SendSolLoadingSheet> createState() => _SendSolLoadingSheetState();
}

enum SendTransactionState { loading, success, error, none }

class _SendSolLoadingSheetState extends State<SendSolLoadingSheet> {
  ...
  send() async {
    try {
      final sign = await TransactionManager.sendSol(receiverAddress: widget.receiverAddress, amountInSol: widget.amountInSol);
      setState(() {
